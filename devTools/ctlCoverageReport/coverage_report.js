 ...
 * 
 * Usage: node coverage_report.js [path] [-f filter] [-v] [-o output.xml]
 */
...
function parseCoverageXml(xmlContent, coverageData, filterPath) {
...
function generateReport(coverageData, verbose) {
  let totalLines = 0;
  let coveredLines = 0;
...
function generateCobertura(coverageData, outputPath) {
  let totalLines = 0;
  let coveredLines = 0;
  let totalBranches = 0;
  let coveredBranches = 0;
...
function findCoverageFiles(dirPath) {
  const files = [];
  try {
...
function main() {
  ...
  const coverageData = { files: new Map() };
