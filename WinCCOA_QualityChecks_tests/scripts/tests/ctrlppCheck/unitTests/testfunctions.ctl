...
//         LOAD_LIB_2(settings.library, "posix.cfg");
    ...
    //TEST_CASE(prohibitedFunctions_std_gets_WinCC_OA); // test std::gets

    TEST_CASE(prohibitedFunctions_multiple); // multiple use of obsolete functions
    //TEST_CASE(prohibitedFunctions_multiple_WinCC_OA); // multiple use of obsolete functions
...
//                       "[test.cpp:4]: (warning) Obsolete function 'gets' called. It is recommended to use 'fgets' or 'gets_s' instead.\n", errout.str());
  ...
  void prohibitedFunctions_std_gets_WinCC_OA() {
    check("struct std{static string gets(string s){return s;}}; \n"
          "void f(string str)\n"
            "{\n"
            "    char x = std::gets(str);\n"
            "    char y = gets(str);\n"
...
//                       "[test.cpp:4]: (style) Obsolescent function 'usleep' called. It is recommended to use 'nanosleep' or 'setitimer' instead.\n", errout.str());
  ...
  /* Makes no sense in Ctrl
  void prohibitedFunctions_multiple_WinCC_OA(){
      check("struct std{static string gets(string s){return s;}}; \n"
            "void f(string str)\n"
            "{\n"
            "    char x = std::gets(str);\n"
...
//         ASSERT_EQUALS("", errout.str());
  ...
  {
    /* Makes no sense in Ctrl (doesnt know memset)
    ...
    //         ASSERT_EQUALS("[test.cpp:2]: (error) Invalid memset() argument nr 3. A non-boolean value is required.\n", errout.str());
    */
    /* Makes no sense in Ctrl (doesnt know strol)
  ...
  void checkIgnoredReturnValue()
  {
    ...
    /* No such warning exists in the source code? maybe a correct xml-file is missing? (The one abouth does not work)
    check("void foo() {\n"
          "  mystrcmp(a, b);\n"
          "}", "test.cpp", settings2);
  ...
  {
    /* Does not make sense in Ctrl (does not know memset)
    check("void f() {\n"
          "    memset(p, 10, 0x0);\n"
          "}\n");
