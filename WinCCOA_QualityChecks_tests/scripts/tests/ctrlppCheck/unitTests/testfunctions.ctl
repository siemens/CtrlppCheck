...
//         ASSERT_EQUALS("[test.cpp:5]: (style) Obsolescent function 'gethostbyaddr' called. It is recommended to use 'getnameinfo' instead.\n", errout.str());
    ...
    void prohibitedFunctions_posix_WinCC_OA() {
        ...
        const string oaLib = getPath(DATA_REL_PATH, "ctrlPpCheck/cfg/ctrl.xml");
        settings.addLibraryFile(oaLib);
        ...
        check("int main(int p)\n"
              ...
              "  DebugN(b);\n"
              "}");
        ASSERT_EQUALS("[test.cpp:3]: (style) Obsolescent function 'isMotif' called. It is recommended to use '' instead.", errout.str());
        ...
        check("void f()\n"
              ...
              "    }\n"
              "}");
        ASSERT_EQUALS("[test.cpp:3]: (style) Obsolescent function 'gethostbyname' called. It is recommended to use 'getaddrinfo' instead.\n", errout.str());
...
//         settings.standards.cpp = Standards::CPP11;
    ...
    void prohibitedFunctions_declaredFunction() {
        check("int ftime ( int a )\n"
              ...
              "    return 0 ;\n"
              "}");
        ASSERT_EQUALS("[test.cpp:7]: (style) Variable 'b' is assigned a value that is never used.", errout.str());
//         ASSERT_EQUALS("", errout.str());
    }
...
//                       "[test.cpp:3]: (portability) Non reentrant function 'crypt' called. For threadsafe applications it is recommended to use the reentrant replacement function 'crypt_r'.\n", errout.str());
    ...
    void prohibitedFunctions_crypt_WInCC_OA() {
        check("void f(char pwd)\n"
              ...
              "    char cpwd = crypt(pwd);\n"
              "}");
        ASSERT_EQUALS("[test.cpp:3]: (style) Variable 'cpwd' is assigned a value that is never used.", errout.str());
    }
    ...
    void prohibitedFunctions_namespaceHandling() {
        check("int f()\n"
              ...
              "    std::localtime(&t);\n"
              "}");
        ASSERT_EQUALS("WinCC OA syntax error at pos: 34", errout.str());
