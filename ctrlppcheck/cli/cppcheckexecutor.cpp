...
bool CppCheckExecutor::parseFromArgs(CppCheck *cppcheck, int argc, const char* const argv[])
{
    ...
    if (_files.empty() && settings.project.fileSettings.empty())
    {
        ...
        std::string detail = !ignored.empty() ? " Maybe all paths were ignored?." : "";
        const std::list<ErrorLogger::ErrorMessage::FileLocation> callstack;
        const std::string msg("Sorry, we can't check the project because there are no scripts or libs inside.\n"
                              "Could not find or open any of the paths given." +
                              detail);
        ErrorLogger::ErrorMessage errmsg(callstack, emptyString, Severity::information, msg, "scriptsListIsEmpty", false);
        reportErr(errmsg);
...
static void CppcheckSignalHandler(int signo, siginfo_t * info, void * context)
{
    ...
    case SIGSEGV:
        ...
        fprintf(output, " (%sat 0x%lx).%s\n",
                // ctrlppcheck-suppress knownConditionTrueFalse ; FP
                (type==-1)? "" :
