...
static simplecpp::DUI createDUI(const Settings &mSettings, const std::string &cfg, const std::string &filename)
{
    ...
    for (auto itr = mSettings.library.defines.begin(); itr != mSettings.library.defines.end(); ++itr) { 
        simplecpp::UserDefinedValue def;
        def.name = itr->second.name;
        def.type = itr->second.type;
        def.value = itr->second.value;
        def.isConst = itr->second.isConst;
        dui.defines[def.name] = def;
    } 
...
simplecpp::TokenList Preprocessor::preprocess(const simplecpp::TokenList &tokens1, const std::string &cfg, std::vector<std::string> &files, bool throwError)
{
    ...
    simplecpp::preprocess(tokens2, tokens1, files, mTokenLists, dui, &outputList);
...
static void getConfigs(const simplecpp::TokenList &tokens, std::set<std::string> &defined, std::set<std::string> &ret)
{
    ...
    for (const simplecpp::Token *tok = tokens.cfront(); tok; tok = tok->next) {
        ...
        if (!sameline(tok, cmdtok))
            continue;
        if (cmdtok->str() == "define" && sameline(tok, cmdtok->next) && cmdtok->next->name) {
