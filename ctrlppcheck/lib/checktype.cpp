...
void CheckType::checkTooBigBitwiseShift()
{
    ...
    for (const Token *tok = mTokenizer->tokens(); tok; tok = tok->next()) {
        // C++ and macro: OUT(x<<y)
        if (Token::Match(tok, "[;{}] %name% (") && Token::simpleMatch(tok->linkAt(2), ") ;") && tok->next()->isUpperCaseName() && !tok->next()->function())
            tok = tok->linkAt(2);
        ...
        //@todo sign - activate when there is ctrl sign
...
void CheckType::checkIntegerOverflow()
{
    ...
    for (const Token *tok = mTokenizer->tokens(); tok; tok = tok->next()) {
        ...
        //@todo sign - activate when there is ctrl sign  || vt->sign != ValueType::Sign::SIGNED
        if (!vt || !vt->isIntegral())
            continue;
...
void CheckType::checkLongCast()
{
    ...
    for (const Token *tok = mTokenizer->tokens(); tok; tok = tok->next()) {
        ...
        if (rhstype->type == ValueType::Type::INT &&
            ...
            lhstype->type == ValueType::Type::LONG &&
        ...
        for (const Token *tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
            if (tok->str() == "return") {
                if (Token::Match(tok->astOperand1(), "<<|*")) {
                    ...
                    if (type && type->type == ValueType::Type::INT && type->originalTypeName.empty())
                        ret = tok;
                }
