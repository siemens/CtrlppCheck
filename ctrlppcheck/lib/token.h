...
public:
    ...
    //-------------------------------------------------------------------------
    /**
     * return unqouted string
     * "bla" returns bla
     * bla return bla
     * "bla return "bla
     * bla" return bla"
     */
    const std::string unquoteStr() const;
    ...
    */
    ...
    /// check of token is relational operator
    /// <=, <, >=, >
    bool isRelationOp() const {
        return (isComparisonOp() && str() != "==" && str() != "!=");
    }
    ...
    /// @todo not sure, but i think we can remove that
    /// ctrl does not support syntax 'long int i;'
    bool isLong() const {
     ...
     * to.
     */
    void setLink(Token *linkToToken) {
      mLink = linkToToken;
      if (mStr == "<" || mStr == ">")
        update_property_info();
    }
    ...
    */
     ...
     * @todo remove this function in ctrl lang, or refactor in case of anytype, mixed variable types
     */
...
private:
    ...
    enum {
      /// @todo fIsUnsigned and fIsSigned can be removed. But it is really mutch effort
        fIsUnsigned             = (1 << 0),
        fIsSigned               = (1 << 1),
        fIsLong                 = (1 << 3),
        fIsStandardType         = (1 << 4),
...
public:
    ...
    static void toStdOut(const Token *tok);
    static void toStdOut(Token *tok);
    ...
    //-------------------------------------------------------------------------
    /** @brief Function check if the token is ctrl key word (if,else,while...).
        @details ctrl keywords are defined in static member controlFlowKeywords
        @return true when token is ctrl keyword. Otherwise false.
    */
    bool isControlFlowKeyword();
};
