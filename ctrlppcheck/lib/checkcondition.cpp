...
bool CheckCondition::isParameterChanged(const Token *partok)
{
    ...
    if (!ftok)
        return true; // implementation error, it shall be never happened
    ...
    if (ftok->function())
    {
        ...
        if (par->isConst())
            return false;
        if (addressOf || par->isReference())
            return true;
        return false;
    }
    else // This is not known function from code. Try to macth with library config.
    {
        const Library::ArgumentChecks::Direction argDirection = mSettings->library.getArgDirection(ftok, 1 + argumentNumber);
        if (!argDirection)
        {
            /// @todo throw error here
            //        std::cout << __FUNCTION__ << " unknown direction " << std::endl;
            return false; // function or argument not found. I think the parameter ca not be changed in that case.
        }

        if (argDirection != Library::ArgumentChecks::Direction::DIR_IN)
        {
            return true; // NOT input param can be changed
        }

        // definitive input only parameter. So it can not be changed by function call
        return true;
    }
}
    ...
    for (const Token *tok = mTokenizer->tokens(); tok; tok = tok->next()) {
        if (tok->str() == "|" && tok->astOperand1() && tok->astOperand2() && tok->astParent()) {
            ...
            const bool isBoolean = Token::Match(parent, "&&|%oror%") ||
                                   ...
                                   (parent->str() == "=" && parent->astOperand2() == tok && parent->astOperand1() && parent->astOperand1()->variable() && Token::Match(parent->astOperand1()->variable()->typeStartToken(), "bool")) ||
    ...
    for (const Token *tok = mTokenizer->tokens(); tok; tok = tok->next()) {
        ...
        for (const MathLib::bigint num1 : numbers) {
            ...
            if (Token::Match(tok, "==|!=")) {
                ...
                { //@todo sign needs ctrl sign check? (expr1->astOperand1()->valueType()->sign == ValueType::Sign::UNSIGNED)
                    const bool or_equal = Token::Match(tok, ">=|<=");
                    const std::string& op(tok->str());
                    if ((Token::Match(tok, ">=|<")) && (num1 >= num2)) {
...
bool CheckCondition::isOverlappingCond(const Token * const cond1, const Token * const cond2) const
{
    if (!cond1 || !cond2)
        return false;
    ...
    // same expressions
    bool pure = true;
    if (isSameExpression(cond1, cond2, mSettings->library, pure, false))
        return true;
    ...
    if (cond1->str() == "&" && cond1->astOperand1() && cond2->astOperand2()) {
        ...
        if (!isSameExpression(expr1, expr2, mSettings->library, pure, false))
            return false;
...
void CheckCondition::duplicateCondition()
{
    ...
    for (const Scope &scope : symbolDatabase->scopeList) {
        ...
        visitAstNodes(cond1, [&](const Token *tok3) {
            if (tok3->varId() > 0 &&
                isVariableChanged(scope.classDef->next(), cond2, tok3->varId(), false, mSettings)) {
                modified = true;
        ...
        ErrorPath errorPath;
        if (!modified &&
            isSameExpression(cond1, cond2, mSettings->library, true, true, &errorPath))
    ...
    for (const Scope &scope : symbolDatabase->scopeList) {
        ...
        for (;;) {
            ...
            if (/*cond1 &&*/
                ...
                isOverlappingCond(cond1, tok2->astOperand2()))
            ...
            if (cond->varId()) {
                ...
                if (!nonlocal && var) {
                    if (!(var->isLocal() || var->isArgument()))
                        nonlocal = true;
                    else if (var->isReference() && !Token::Match(cond->astParent(), "%oror%|&&|!"))
                        // TODO: if var is pointer check what it points at
                        nonlocal = true;
                }
            } else if (!nonlocal && cond->isName()) {
                // varid is 0. this is possibly a nonlocal variable..
                nonlocal = Token::Match(cond->astParent(), "%cop%|(|[") || Token::Match(cond, "%name% .") || cond->str() == "this";
            } else {
                return ChildrenToVisit::op1_and_op2;
            }
            ...
            for (; tok && tok != endToken; tok = tok->next()) {
                    ...
                    if (type == MULTICONDITIONTYPE::INNER) {
                        ...
                        while (!tokens1.empty()) {
                            ...
                            if (firstCondition->str() == "&&") {
                                ...
                                } else if (!isReturnVar && isSameExpression(firstCondition, cond2, mSettings->library, true, true, &errorPath)) {
                        ...
                        while (!tokens2.empty()) {
                            ...
                            if (secondCondition->str() == "||" || secondCondition->str() == "&&") {
                                ...
                                tokens2.push(secondCondition->astOperand2());
                                       ...
                                       isSameExpression(cond1, secondCondition, mSettings->library, true, true, &errorPath)) {
                                if (!isAliased(vars))
                ...
                if (Token::Match(tok, "for|while|do")) {
                    ...
                    for (unsigned int varid : vars) {
                        if (isVariableChanged(tok1, tok2, varid, nonlocal, mSettings)) {
                            changed = true;
                            break;
                        }
                    }
...
void CheckCondition::checkIncorrectLogicOperator()
{
    ...
    for (const Scope * scope : symbolDatabase->functionScopes) {
        ...
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if (!isfloat && isOppositeCond(tok->str() == "||", tok->astOperand1(), tok->astOperand2(), mSettings->library, true, true, &errorPath)) {
            ...
            if (isSameExpression(comp1, comp2, mSettings->library, true, true))
                continue; // same expressions => only report that there are same expressions
            if (!isSameExpression(expr1, expr2, mSettings->library, true, true))
                continue;
    ...
    for (const Scope * scope : symbolDatabase->functionScopes) {
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if (Token::Match(tok, "%oror%|&&|:"))
                continue;
            if (Token::Match(tok, "%comp%") && isSameExpression(tok->astOperand1(), tok->astOperand2(), mSettings->library, true, true))
                continue;
            ...
            if (returnStatement && Token::simpleMatch(tok->astParent(), "return") && tok->variable() && (
                    ...
                    tok->variable()->isConst() ||
                    !isVariableChanged(tok->variable(), mSettings)))
                continue;
...
void CheckCondition::alwaysTrueFalseError(const Token *tok, const ValueFlow::Value *value)
{
    ...
    const std::string errmsg = "Condition is always " + (std::string)(condvalue ? "true" : "false") + ": " + expr;
...
void CheckCondition::checkInvalidTestForOverflow()
{
    ...
    for (const Scope * scope : symbolDatabase->functionScopes) {
        ...
        for (const Token* tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
            ...
            // Only warn for signed integer overflows overflows.
            //@todo sign - actiavte when there is ctrl sign
            /*if (!(calcToken->valueType() && (calcToken->valueType()->sign == ValueType::Sign::SIGNED)))
                continue;
               if (!(exprToken->valueType() && (exprToken->valueType()->sign == ValueType::Sign::SIGNED)))
                continue;*/

            const Token *termToken;
            if (isSameExpression(exprToken, calcToken->astOperand1(), mSettings->library, true, false))
                termToken = calcToken->astOperand2();
            else if (isSameExpression(exprToken, calcToken->astOperand2(), mSettings->library, true, false))
            ...
            if (termToken->isNumber() && MathLib::isPositive(termToken->str()))
                invalidTestForOverflow(tok, result);
        }
    }
}
