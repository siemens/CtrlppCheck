...
class Variables {
public:
    enum VariableType { standard, array, reference, referenceArray, none };
...
static const Token* doAssignment(Variables &variables, const Token *tok, bool dereference, const Scope *scope)
{
    ...
    if (var1) {
        ...
        if (Token::Match(tok, "( const| struct| %type% * ) ( ("))
            tok = tok->link()->next();
        ...
        if (Token::Match(tok, "(| &| %name%") ||
            (Token::Match(tok->next(), "< const| struct| %type% *| > ( &| %name%"))) {
            ...
            if (var2) { // local variable (alias or read it)
                if (var1->mType == Variables::reference)
                {
...
static bool isPartOfClassStruct(const Token *tok)
{
    for (; tok; tok = tok->previous()) {
        if (tok->str() == "}" || tok->str() == ")")
            ...
            return (tok->strAt(-1) == "struct" || tok->strAt(-2) == "struct" || tok->strAt(-1) == "class" || tok->strAt(-2) == "class");
        }
    }
    return false;
}
...
static void useFunctionArgs(const Token *tok, Variables& variables)
{
    ...
    } else if (Token::Match(tok, "[+:]") && !tok->valueType()) {
        useFunctionArgs(tok->astOperand1(), variables);
        useFunctionArgs(tok->astOperand2(), variables);
...
void CheckUnusedVar::checkFunctionVariableUsage_iterateScopes(const Scope* const scope, Variables& variables)
{
    ...
    if (scope->isExecutable()){
        ...
        for (std::list<Variable>::const_iterator i = scope->varlist.begin(); i != scope->varlist.end(); ++i) {
            ...
            if (i->isArray() && i->nameToken()->previous()->str() == "&")
                type = Variables::referenceArray;
            else if (i->isArray())
                type = Variables::array;
            else if (i->isReference())
                type = Variables::reference;
            else if (i->typeEndToken()->isStandardType() ||
                     isRecordTypeWithoutSideEffects(i->type()))
                type = Variables::standard;
            if (type == Variables::none || isPartOfClassStruct(i->typeStartToken()))
                continue;
    ...
    for (; tok && tok != scope->bodyEnd; tok = tok->next()) {
        ...
        /// @todo i think this block makes no sense for ctrl lang
        // assignment
            ...
            const bool deref = eq && eq->astOperand1() && eq->astOperand1()->valueType();
            ...
            if (var) {
                ...
                if ( var->mType == Variables::reference)
                {
                    variables.read(varid, tok);
...
static bool shallCheckExpressionUsage(const Token *tok)
{
    if ( !tok )
    {
        // defensive
    ...
    // when token == . it means we shall check the ast operand of him
    // ex:
    // shapeName.property or object.member
    if ( tok->str() == "." && tok->astOperand1() )
    {
        return shallCheckExpressionUsage(tok->astOperand1());
    }

    const Variable *var = tok->variable();
    if ( !var )
    {
        // this check works only for variables
        return false;
    }
    const ValueType *type = var->valueType();
    if ( !type ||  !type->isKnownType() )
    {
        // ignore unkown variable types.
        return false;
    }
...
void CheckUnusedVar::checkFunctionVariableUsage()
{
    ...
    for (const Scope * scope : symbolDatabase->functionScopes) {
        ...
        for (const Token *tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if ( !shallCheckExpressionUsage(expr) )
            {
                continue;
            }
        ...
        for (std::map<unsigned int, Variables::VariableUsage>::const_iterator it = variables.varUsage().begin();
            ...
            if (usage.mType == Variables::referenceArray)
                continue;
            ...
            else if (!usage._write && !usage._allocateMemory && var && !isEmptyType(var->type()))
                unassignedVariableError(usage._var->nameToken(), varname);
        }
    }
}
...
{
    reportError(tok, Severity::style, "unusedAllocatedMemory", "$symbol:" + varname + "\nVariable is allocated memory that is never used: '$symbol'", CWE563, false);
}

void CheckUnusedVar::unreadVariableError(const Token *tok, const std::string &varname, bool modified)
{
    if (modified)
        reportError(tok, Severity::warning, "unreadVariable", "$symbol:" + varname + "\nVariable is modified but it's new value is never used: '$symbol'", CWE563, false);
    else
        reportError(tok, Severity::warning, "unreadVariable", "$symbol:" + varname + "\nVariable is assigned a value that is never used: '$symbol'", CWE563, false);
}

void CheckUnusedVar::unassignedVariableError(const Token *tok, const std::string &varname)
{
    reportError(tok, Severity::style, "unassignedVariable", "$symbol:" + varname + "\nVariable is not assigned a value: '$symbol'", CWE665, false);
}
...
void CheckUnusedVar::checkStructMemberUsage()
{
    ...
    for (const Scope &scope : symbolDatabase->scopeList) {
        ...
        // Bail out if struct contains any functions
        ...
        for (const Variable* var : symbolDatabase->variableList()) {
            if (var && (var->isGlobal() && !var->isStatic()) && var->typeEndToken()->str() == scope.className) {
                bailout = true;
                break;
            }
        }
        ...
        for (const Variable &var : scope.varlist) {
            ...
            unusedStructMemberError(var.nameToken(), scope.className, var.name());
        }
    }
}
...
{
    const std::string prefix = "struct member ";
