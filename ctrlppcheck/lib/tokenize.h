...
public:
    ...
    */
     ...
     * assigned variable.
     */
    static bool simplifyKnownVariablesGetData(unsigned int varid, Token **_tok2, Token **_tok3, std::string &value, unsigned int &valueVarId, bool floatvar);

    /**
     * utility function for simplifyKnownVariables. Perform simplification
     * of a given variable
     */
    bool simplifyKnownVariablesSimplify(Token **tok2, Token *tok3, unsigned int varid, const std::string &structname, std::string &value, unsigned int valueVarId, const Token * const valueToken, int indentlevel) const;
     ...
     */
    //@todo Refactoring: must be changed
     ...
     * @return token matching with endsWith if syntax seems to be a function head else nullptr
     */
    static const Token * isFunctionHead_static(const Token *tok, const std::string &endsWith);
...
public:
    ...
    void syntaxError(const Token *tok, const std::string &detail) const;
    // void syntaxError(const Token *tok, const std::string &file, int line, const std::string &function) const;

    // __FILE__, __LINE__, __func__
    void syntaxErrorDetail(const Token *tok) const;
    static void syntaxErrorDetailMsg(const Token *tok, std::string detail);
...
private:
    ...
    */
    ...
    CalculationsSimplifier *mCalculationsSimplifier;
