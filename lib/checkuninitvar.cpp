...
void CheckUninitVar::checkScope(const Scope* scope, const std::set<std::string> &arrayTypeDefs)
{
    for (const Variable &var : scope->varlist) {
        ...
        bool stdtype = arrayTypeDefs.find(var.typeStartToken()->str()) == arrayTypeDefs.end();
...
bool CheckUninitVar::checkScopeForVariable(const Token *tok, const Variable& var, bool * const possibleInit, bool * const noreturn, Alloc* const alloc, const std::string &membervar, std::map<unsigned int, VariableValue> variableValue)
{
    ...
    for (; tok; tok = tok->next()) {
        ...
        // variable is seen..
        if (tok->varId() == var.declarationId()) {
            if ((var.typeStartToken()->isStandardType() || var.typeStartToken()->isEnumType() || (var.type() && var.type()->needInitialization == Type::True)) && Token::simpleMatch(tok->next(), "= new")) {
                *alloc = CTOR_CALL;
...
bool CheckUninitVar::isVariableUsage(const Token *vartok, Alloc alloc) const
{
    ...
    // Passing variable to function..
    {
        ...
        if (Token::Match(possibleParent, "[(,]")) {
            ...
            const int use = isFunctionParUsage(vartok, alloc);
            if (use >= 0)
                return (use>0);
        }

        else if (Token::simpleMatch(possibleParent, "=") && vartok->astParent()->str() == "&") {
            return false;
        }
    }

    if (Token::Match(vartok->previous(), "++|--|%cop%")) {
        if (alloc == ARRAY && Token::Match(vartok->tokAt(-4), "& %var% =|( *"))
            return false;
        ...
        if (vartok->next() && vartok->next()->str() == "(")
            return false;
    ...
    if (alloc != NO_ALLOC && Token::Match(vartok, "%var% [")) {
 ...
 */
int CheckUninitVar::isFunctionParUsage(const Token *vartok, Alloc alloc) const
{
    bool unknown = false;
    ...
    if (Token::Match(start->previous(), "%name% (")) {
        ...
        if (func) {
            ...
            if (arg) {
                ...
                if (address && Token::Match(argStart, "const %type% %name% [") && Token::Match(argStart->linkAt(3), "] [,)]"))
                    return 1;
            }
    ...
    if (Token::Match(tok, "%name% . %name%") && tok->strAt(2) == membervar) {
        if (Token::Match(tok->tokAt(3), "[=.[]"))
            ...
            ; // member variable usage
                 ...
                 1 == isFunctionParUsage(tok, NO_ALLOC)) {
            return false;
        } else
            return true;
    } else if (tok->strAt(1) == "=")
...
{
    reportError(tok, Severity::information, "uninitdata", "$symbol:" + varname + "\nMemory is allocated but not initialized: $symbol", CWE908, false);
}

void CheckUninitVar::uninitvarError(const Token *tok, const std::string &varname)
{
    reportError(tok, Severity::information, "uninitvar", "$symbol:" + varname + "\nUninitialized variable: $symbol", CWE908, true);
}
...
static bool isVariableUsage(const Check *check, const Token *vartok)
{
    ...
    return c && c->isVariableUsage(vartok, CheckUninitVar::Alloc::ARRAY);
}
