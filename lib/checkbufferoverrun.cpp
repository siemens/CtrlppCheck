...
void CheckBufferOverrun::arrayIndex()
{
    for (const Token *tok = mTokenizer->tokens(); tok; tok = tok->next())
    {
        ...
        MathLib::bigint dim = 0;


        if (stringLiteral || !array->variable()->dimensions().empty()) //workarround for arr with empty dimensions
        {
            dim = stringLiteral ? Token::getStrSize(stringLiteral) : array->variable()->dimensions()[0].num;
        }

        // Positive index
        if (stringLiteral || dim >= 1)
        { // TODO check arrays with dim 1 also
        ...
        for (const Token *tok = scope->bodyStart; tok && tok != scope->bodyEnd; tok = tok->next())
        {
            ...
            for (const Token *tok2 = tok->next()->link(); tok2 != scope->bodyEnd; tok2 = tok2->next())
            {
                ...
                if (!rhs || !rhs->hasKnownIntValue() || rhs->getKnownIntValue() != 0)
                    continue;
                if (isSameExpression(args[0], tok2->link()->astOperand1(), mSettings->library, false, false))
...
void CheckBufferOverrun::bufferNotZeroTerminatedError(const Token *tok, const std::string &varname, const std::string &function)
{
    const std::string errmsg = "$symbol:" + varname + '\n' +
                               ...
                               "The buffer '" + varname + "' is not null-terminated after the call to " + function + "().\n"
                                                                                                                     "The buffer '" +
                               varname + "' is not null-terminated after the call to " + function + "(). "
