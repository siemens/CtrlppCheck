...
bool astIsFloat(const Token *tok, bool unknown)
{
    ...
    if (!vt)
        return unknown;
    return vt->type >= ValueType::Type::FLOAT;
}
    ...
    for (const Token * tok2 = startToken; tok2 != endToken; tok2 = tok2->next()) {
        ...
        if (const Variable * var2 = tok2->variable()) {
            ...
            if (var2->isStatic() && !var2->isConst())
                return tok;
            if (!var2->isConst() && (!precedes(tok2, endToken2) || isVariableChanged(tok2, endToken2, tok2->varId(), false, nullptr)))
                return tok;
            ...
            return tok;
        } else if (tok2->tokType() == Token::eName && !tok2->function()) {
            return tok;
        }
    }
    return varTok;
...
bool isSameExpression(const Token *tok1, const Token *tok2, const Library &library, bool pure, bool followVar, ErrorPath* errors)
{
    ...
    if (Token::simpleMatch(tok1, "!") && Token::simpleMatch(tok1->astOperand1(), "!") && !Token::simpleMatch(tok1->astParent(), "=")) {
        return isSameExpression(tok1->astOperand1()->astOperand1(), tok2, library, pure, followVar, errors);
    }
    if (Token::simpleMatch(tok2, "!") && Token::simpleMatch(tok2->astOperand1(), "!") && !Token::simpleMatch(tok2->astParent(), "=")) {
        return isSameExpression(tok1, tok2->astOperand1()->astOperand1(), library, pure, followVar, errors);
    }
    // Follow variable
    if (followVar && tok1->str() != tok2->str() && (Token::Match(tok1, "%var%") || Token::Match(tok2, "%var%"))) {
        const Token * varTok1 = followVariableExpression(tok1, tok2);
        if (varTok1->str() == tok2->str()) {
            followVariableExpressionError(tok1, varTok1, errors);
            return isSameExpression(varTok1, tok2, library, true, followVar, errors);
        }
        const Token * varTok2 = followVariableExpression(tok2, tok1);
        if (tok1->str() == varTok2->str()) {
            followVariableExpressionError(tok2, varTok2, errors);
            return isSameExpression(tok1, varTok2, library, true, followVar, errors);
        }
        if (varTok1->str() == varTok2->str()) {
            followVariableExpressionError(tok1, varTok1, errors);
            followVariableExpressionError(tok2, varTok2, errors);
            return isSameExpression(varTok1, varTok2, library, true, followVar, errors);
        }
    }
    if (tok1->varId() != tok2->varId() || tok1->str() != tok2->str() || tok1->originalName() != tok2->originalName()) {
        if ((Token::Match(tok1,"<|>")   && Token::Match(tok2,"<|>")) ||
            (Token::Match(tok1,"<=|>=") && Token::Match(tok2,"<=|>="))) {
            return isSameExpression(tok1->astOperand1(), tok2->astOperand2(), library, pure, followVar, errors) &&
                   isSameExpression(tok1->astOperand2(), tok2->astOperand1(), library, pure, followVar, errors);
        }
        return false;
    }
    ...
    if (tok1->isSigned() != tok2->isSigned())
        return false;
    if (pure && tok1->isName() && tok1->next()->str() == "(") {
        if (!tok1->function()) {
    ...
    bool noncommutativeEquals =
        isSameExpression(tok1->astOperand1(), tok2->astOperand1(), library, pure, followVar, errors);
    noncommutativeEquals = noncommutativeEquals &&
                           isSameExpression(tok1->astOperand2(), tok2->astOperand2(), library, pure, followVar, errors);
    ...
    commutativeEquals = commutativeEquals &&
                        isSameExpression(tok1->astOperand1(), tok2->astOperand2(), library, pure, followVar, errors);
...
static bool isZeroBoundCond(const Token * const cond)
{
    ...
    const bool isZero = (cond->astOperand2()->getValue(0) !=  nullptr);
...
bool isOppositeCond(bool isNot, const Token * const cond1, const Token * const cond2, const Library& library, bool pure, bool followVar, ErrorPath* errors)
{
    ...
    if (cond1->str() == "!") {
        if (cond2->str() == "!=") {
            if (cond2->astOperand1() && cond2->astOperand1()->str() == "0")
                return isSameExpression(cond1->astOperand1(), cond2->astOperand2(), library, pure, followVar, errors);
            if (cond2->astOperand2() && cond2->astOperand2()->str() == "0")
                return isSameExpression(cond1->astOperand1(), cond2->astOperand1(), library, pure, followVar, errors);
        }
        return isSameExpression(cond1->astOperand1(), cond2, library, pure, followVar, errors);
    }
    ...
    if (!isNot) {
        if (cond1->str() == "==" && cond2->str() == "==") {
            if (isSameExpression(cond1->astOperand1(), cond2->astOperand1(), library, pure, followVar, errors))
                return isDifferentKnownValues(cond1->astOperand2(), cond2->astOperand2());
            if (isSameExpression(cond1->astOperand2(), cond2->astOperand2(), library, pure, followVar, errors))
    ...
    std::string comp2;
    if (isSameExpression(cond1->astOperand1(), cond2->astOperand1(), library, pure, followVar, errors) &&
        isSameExpression(cond1->astOperand2(), cond2->astOperand2(), library, pure, followVar, errors)) {
        comp2 = cond2->str();
    } else if (isSameExpression(cond1->astOperand1(), cond2->astOperand2(), library, pure, followVar, errors) &&
             isSameExpression(cond1->astOperand2(), cond2->astOperand1(), library, pure, followVar, errors)) {
        comp2 = cond2->str();
        if (comp2[0] == '>')
            comp2[0] = '<';
        else if (comp2[0] == '<')
    ...
    if (!isNot && comp2.empty()) {
        ...
        if (!isSameExpression(expr1, expr2, library, pure, followVar, errors))
            return false;
...
bool isOppositeExpression(const Token * const tok1, const Token * const tok2, const Library& library, bool pure, bool followVar, ErrorPath* errors)
{
    ...
    if (isOppositeCond(true, tok1, tok2, library, pure, followVar, errors))
        return true;
    if (tok1->isUnaryOp("-"))
        return isSameExpression(tok1->astOperand1(), tok2, library, pure, followVar, errors);
    if (tok2->isUnaryOp("-"))
        return isSameExpression(tok2->astOperand1(), tok1, library, pure, followVar, errors);
    return false;
}
    ...
    for (const Token *tok = start; tok != end; tok = tok->next()) {
        ...
        if (Token::Match(tok, "%name% . %name% (")) {
            ...
            if (!isConst && var) {
                ...
                isConst = (valueType && valueType->constness == 1);
            }
...
bool isVariableChanged(const Variable * var, const Settings *settings)
{
    if (!var)
        return false;
    if (!var->scope())
    ...
    if (Token::Match(start, "; %varid% =", var->declarationId()))
        start = start->tokAt(2);
    return isVariableChanged(start->next(), var->scope()->bodyEnd, var->declarationId(), var->isGlobal(), settings);
}
...
struct FwdAnalysis::Result FwdAnalysis::checkRecursive(const Token *expr, const Token *startToken, const Token *endToken, const std::set<unsigned int> &exprVarIds, bool local)
{
    ...
    for (const Token *tok = startToken; tok != endToken; tok = tok->next()) {
        ...
        if (exprVarIds.find(tok->varId()) != exprVarIds.end()) {
            ...
            bool other = false;
            bool same = tok->astParent() && isSameExpression(expr, tok, mLibrary, false, false, nullptr);
            while (!same && Token::Match(parent->astParent(), "*|.|::|[")) {
                parent = parent->astParent();
                if (parent && isSameExpression(expr, parent, mLibrary, false, false, nullptr)) {
                    same = true;
            ...
            if (Token::simpleMatch(parent->astParent(), "=") && parent == parent->astParent()->astOperand1()) {
                ...
                const bool reassign = isSameExpression(expr, parent, mLibrary, false, false, nullptr);
                if (reassign)
    ...
    [&](const Token *tok) {
        ...
        if (tok->originalName() == "->") {
            ...
            if (lhsvar->isArgument() && lhsvar->isArray()) {
                globalData = true;
                return ChildrenToVisit::none;
            } else if (lhsvar->isArgument() && (!lhstype || lhstype->type <= ValueType::Type::VOID)) {
                globalData = true;
                return ChildrenToVisit::none;
            }
        }
        ...
        if (tok->str() == "." && tok->astOperand1() && tok->astOperand1()->variable() && !tok->astOperand1()->valueType()) {
            globalData = true;
            return ChildrenToVisit::none;
        }
        if (Token::Match(tok, ".|["))
...
bool FwdAnalysis::hasOperand(const Token *tok, const Token *lhs) const
{
    if (!tok)
        return false;
    if (isSameExpression(tok, lhs, mLibrary, false, false, nullptr))
        return true;
...
bool FwdAnalysis::possiblyAliased(const Token *expr, const Token *startToken) const
{
    ...
    for (const Token *tok = startToken; tok; tok = tok->previous()) {
        ...
        if (Token::Match(tok, "%name% (") && !Token::Match(tok, "if|while|for")) {
            ...
            for (unsigned int argnr = 0; argnr < args.size(); ++argnr) {
                ...
                for (const Token *subexpr = expr; subexpr; subexpr = subexpr->astOperand1()) {
                    if (isSameExpression(subexpr, args[argnr], mLibrary, pure, followVar))
                        return true;
                }
            }
            continue;
        ...
        for (const Token *subexpr = expr; subexpr; subexpr = subexpr->astOperand1()) {
            if (isSameExpression(subexpr, addrOf, mLibrary, pure, followVar))
                return true;
        }
    }
    return false;
