...
void CheckString::checkSuspiciousStringCompare()
{
    ...
    for (const Scope * scope : symbolDatabase->functionScopes) {
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if (varTok->str() == "*") {
                if (varTok->astOperand2() != nullptr || litTok->tokType() != Token::eString)
                    continue;
...
{
    return (var && !var->isArray() && var->typeStartToken()->str() == "char");
}
            ...
            for (const Token *eq0 : equals0) {
                for (const Token * ne0 : notEquals0) {
                    ...
                    if (args1[1]->isLiteral() &&
                        ...
                        isSameExpression(args1[0], args2[0], mSettings->library, true, false))
    ...
    for (const Scope * scope : symbolDatabase->functionScopes) {
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            ...
            for (unsigned int argnr = formatString + 1; argnr < args.size(); ++argnr) {
                ...
                const bool same = isSameExpression(dest,
                                                   arg,
                                                   mSettings->library,
                                                   true,
                                                   false);
