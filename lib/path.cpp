...
{
  return false; // make Non-windows filesystems happy, might be case insensitive
}
...
bool Path::isCTRL(const std::string &path)
{
...
{
    return Path::isCTRL(path)  || extra.find(getFilenameExtension(path)) != extra.end();
}
